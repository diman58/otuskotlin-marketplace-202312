...
class Exercise1Filter {
    ...
    fun filter() = runBlocking {
        ...
        val res = LIST
            .asFlow()
            .run { flt.title?.let { title -> this.filter { it.title == title } } ?: this }
            .run { flt.type?.let { type -> this.filter { it.type == type } } ?: this }
            .run { flt.visibilitiesOr?.let { adVisibilities: Set<AdVisibility> -> this.filter {
                it.visibility == adVisibilities.first { ad -> ad == AdVisibility.OWNER } } } ?: this }
            .toList()
